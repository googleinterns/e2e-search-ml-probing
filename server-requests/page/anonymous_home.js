...
class AnonymousHome extends PotatoBase {
  class_name() { return "AnonymousHome"; }
  ...
  static async New(tab) {
    ...
    await p.init(
        tab, '//*[@id="text" and contains(text(), "Sign in")]');
    return p;
  }
...
class UsernamePrompt extends PotatoBase {
  class_name() { return "anonymous_home.UsernamePrompt"; }
  ...
  async enterUsername(username) {
    ...
    const naving = this.tab().waitForNavigation({waitUntil: 'networkidle2'});
...
class PasswordPrompt extends PotatoBase {
  class_name() { return "anonymous_home.PasswordPrompt"; }
  ...
  async enterPassword(password) {
    ...
    try { await pwCoverMessage.click(); } catch (e) { this.log('click1 failed: ' + e); }
    try { await pwField.click(); } catch (e) { this.log('click2 failed: ' + e); }
