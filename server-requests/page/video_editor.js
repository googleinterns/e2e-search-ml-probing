...
class VideoEditor extends PotatoBase {
  ...
  async openAudioEditTab() {
    ...
    let audioEditPagePromise =
        new Promise(resolve => this.tab().once('popup', resolve));
  ...
  async trimToRandomDuration() {
    ...
    const randomTargetX =
        leftCenterX + (rightCenterX - leftCenterX) *
                      (Math.random() * 0.6 + 0.2);
    await this.tab().mouse().move(
        rightCenterX, rightCenterY, {steps: 10});
    ...
    await this.tab().mouse().click(randomTargetX, rightBox.y - 15);
    const playhead = await this.tab().waitForXPath(
        '//ytve-playhead[@id="playhead"]');
    const playheadTime =
        await playhead.evaluate(x => x.getAttribute("aria-label"));
