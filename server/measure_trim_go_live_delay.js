...
class MeasureTrimGoLiveDelay extends Base {
  class_name() { return "MeasureTrimGoLiveDelay"; }
  ...
  async run() {
    ...
    const {urlVideoId, studioVideos} = await loginAndUpload(
        normalTab, config.username, config.password,
        videoFilePath, videoTitle, videoDescription);
    ...
    try {
      ...
      const studioEditVideo =
          await studioVideos.clickVideoWithUrlVideoId(urlVideoId);
      const videoEditor = await studioEditVideo.goToEditor();

      this.log("Perform random trim between 20% and 80% of total duration.");
      ...
      await this.waitForVideoHasDuration(incogTab, urlVideoId, newDuration_s);
    } finally {
  ...
  async waitForVideoHasDuration(tab, urlVideoId, newDuration_s) {
    ...
    const result = await backoff.exponential({
      initialBackoff_s: Math.max(32, config.waitForUpdateInitialBackoff_s),
      ...
      attemptFunc: async () => {
        ...
        if (Math.abs(currentDuration_s - newDuration_s) <= durationFudge_s) {
          this.log("  Found correct duration.");
          return true;
        }
        this.log("  Found wrong duration: found=" +
    ...
    if (result.ok) {
      this.log("  Successfully found correct length after: " +
               `${result.delay_s}s (${result.delay_s/60.0}m).`);
    } else {
      this.log("  Correct length never found to go live after: " +
               `${result.delay_s}s (${result.delay_s/60.0}m).`);
    }
  }
};
