...
class MeasureDescriptionUpdateDelay extends Base {
  class_name() { return "MeasureDescriptionUpdateDelay"; }
  ...
  async waitForDescriptionWithBackoff(args) {
    ...
    if (result.ok) {
      this.log("  Successfully found new description with delay: " +
               `${result.delay_s}s (${result.delay_s/60.0}m).`);
    } else {
      this.log("  New description never found to go live after: " +
               `${result.delay_s}s (${result.delay_s/60.0}m).`);
    }
  }
};
