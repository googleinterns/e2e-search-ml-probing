...
class App {
  ...
  async run() {
    ...
    if (rematch == null || rematch.length != 2) {
      throw new Error("Description isn't as expected: " + existingDescription);
    }
    ...
    let addToVideoButtons =
        await audioEditPage.page().$x(addToVideoButtonsXPath);
    ...
    let randomAddVideoButton =
        addToVideoButtons[Math.round(Math.random() * addToVideoButtons.length)];
