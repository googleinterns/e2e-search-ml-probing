  ...
  async uploadVideo(args) {
    ...
    for (let i = 0; i < 2 && !secondUploadButton; ++i) {
      try {
        ...
        secondUploadButton = await this.tab().waitForXPath(secondUploadButtonXPath, {
    ...
    const uploadPrompt =
        await studio_videos.UploadPrompt.New(this.giveAwayTab());
