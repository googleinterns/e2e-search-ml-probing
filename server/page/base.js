...
class PotatoBase extends Base {
	...
	tab() {
		if (this.tab_ == null) {
			throw new Error(
				"Potato used with this.tab_ == null.  Likely," +
	...
	log(msg) {
		console.log(this.class_name() + ": " + msg)
	}
	...
	async clickButton(arg) {
		...
		if (arg.expectNav) {
			await this.clickButtonExpectingNav_(
				arg.xpath,
				arg.tab || this.tab()
			)
		} else {
			await this.clickButtonExpectingNoNav_(
				arg.xpath,
				arg.tab || this.tab()
			)
		}
	...
	async clickButtonExpectingNoNav_(xpath, tab) {
		...
		const naving = tab
			.waitForNavigation({ timeout: config.dontNavTimeout_ms })
			.catch(() => {})
		await button.click()
		try {
			await naving
