...
class EditVideoMetadata extends PotatoBase {
  class_name() { return "EditVideoMetadata"; }
  ...
  async setDescription(newContent) {
    await this.descriptionBox_.evaluate(x => { x.textContent = ""; });
    await this.descriptionBox_.type(newContent);
  }

  async clickSave() {
    this.log("Click 'Save' button.");
    await this.clickButton('//ytcp-button[@label="Save"]//*[contains(text(), "Save")]');
  ...
  async videoEditor() {
    ...
    await this.clickButton('//a[@tooltip-text="Editor" and contains(@href, "/editor")]');
