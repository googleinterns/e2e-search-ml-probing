...
class AnonymousSearchResults extends PotatoBase {
  class_name() { return "AnonymousSearchResults"; }

  static async New(tab) {
    const p = new AnonymousSearchResults();
    await p.init(
      tab,
      '//ytd-search[@role="main"]//div[@id="filter-menu"]'
    );
    return p;
  }
  ...
  async getResults() {
    ...
    for (const resultLink of resultLinks) {
      results.push(await resultLink.evaluate(x => {
        return { href: x.href,
                 title: x.querySelector('yt-formatted-string').textContent };
      }));
    }
    return results.map(x => {
      const rematch = x.href.match(/.*\/watch\?v=(.*)$/);
      if (rematch == null || rematch.length != 2 || rematch[1].length != 11) {
