...
class StudioVideos extends PotatoBase {
  class_name() { return "studio_videos.StudioVideos"; }
  ...
  async getUrlVideoIdForTitle(title) {
    ...
    const matching = listed.filter(x => x.title === title)
                           .map(x => x.urlVideoId);
  ...
  async getVisibilityWithTitleToken(titleToken) {
    ...
    for (const row of rows) {
      const rowVideoTitle = await row.evaluate(
          r => r.querySelector('a#video-title').textContent.trim());
    ...
    if (matchingRows.length > 1) {
      throw new Error("More than one title with token found in video list.");
    }

    return await matchingRows[0].evaluate(r => r.querySelector(
        "div.tablecell-visibility").textContent.replace(/\W/g, ""));
  }
};
...
class UploadPrompt extends PotatoBase {
  class_name() { return "studio_videos.UploadPrompt"; }
  static verifyXPath() { return '//*[@id="select-files-button"]//*[contains(text(), "Select file")]'; }
      ...
      python.on('close', async () => {
        ...
        try {
          if (fs.existsSync(pathVideo) && fs.existsSync(pathImg)) {
            ...
            reject("Error on creating the video, check that gen_video.py is working");
          }
        } catch(err) {
          reject("Error on creating the video, check that gen_video.py is working");
        }
        ...
        const configUpload = await ConfigureNewUpload.New(this.giveAwayTab());
...
class ConfigureNewUpload extends PotatoBase {
  class_name() { return "studio_videos.ConfigureNewUpload"; }
  ...
  async configure(args) {
    ...
    const titleBox = await this.tab().waitForXPath(
        '//div[@id="textbox" and @aria-label="Add a title that describes your video"]');
    await titleBox.type('flaky race workaround, type garbage then replace it');
