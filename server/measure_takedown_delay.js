...
const videoFilePath = path.resolve(
	"./test_videos/beatles_takedown_test_video.mp4"
)

class MeasureTakeDownDelay extends Base {
	class_name() {
		return "MeasureTakeDownDelay"
	}
	...
	async run() {
		...
		const personalHome = await anonHome.login(
			config.username,
			config.password
		)
	...
	async waitToShowAsBlocked(studioVideos, titleToken) {
		...
		const result = await backoff.exponential({
			...
			attemptFunc: async () => {
				attemptNum += 1
				const visibility = await studioVideos.getVisibilityWithTitleToken(
					titleToken
				)
				switch (visibility) {
					case "Blocked":
		...
		if (result.ok) {
			this.log(
				"Successfully found 'Blocked' visibility after: " +
					`${result.delay_s}s (${result.delay_s / 60.0}m)`
			)
		} else {
			this.log(
				"Failure, video still not 'Blocked' after: " +
					`${result.delay_s}s (${result.delay_s / 60.0}m)`
			)
		}
	}
	...
	async deleteToCleanUp(studioVideos, urlVideoId) {
		...
		const editVideo = await studioVideos.clickVideoWithUrlVideoId(
			urlVideoId
		)
